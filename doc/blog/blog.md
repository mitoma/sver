...
この手法は、大規模で効率的なビルドをするなら Bazel が良いかもしれない。  
...
GitHub Actions で sver コマンドをインストールしたりするには [setup_sver/action.yaml][] が、sver を用いて artifacts があるときにはジョブをスキップする処理の作りこみには [exec_sver/action.yaml][] が参考になります。  
